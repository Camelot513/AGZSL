...
class data_loader_virtualCls(data.Dataset):
        ...
        def __our_getitem__(self, index, netae):
                is_first = True
                select_feats = []
                select_hs1 = None
                select_atts = []
                ...
                for i in range(len(selected_classes)):
                        ...
                        if i<self.ways/2:
                            ...
                            for j in range(self.shots):
                                    ...
                                    feat = feat.unsqueeze(0)
                                    select_feats_examples = select_feats[int(i - self.ways / 2) * self.shots + j].unsqueeze(0)

                                    _, _, select_hs1_examples, _ = netae(select_feats_examples)

                                    _, _, hs1,_ = netae(feat)
                                    hs1 = lam * hs1 + (1 - lam) * select_hs1_examples
                                    ...
                                    if select_hs1 ==None:
                                        ...
                                        select_hs1 = torch.cat((select_hs1.cuda(), hs1.cuda()), 0)
                                    select_atts = torch.cat((select_atts, att_v),0)
                                    select_labels[i*self.shots+j] = i - int(self.ways/2)

                # print(select_hs1.shape)
                noval_index = int(self.ways/2)*self.shots
                # print(noval_index)

                return select_hs1, select_atts[noval_index:], select_labels[noval_index:]
        ...
        def __train_sdnet__(self, index, netm, netae, neta, ae_optimizer, optimizer_neta):
                is_first = True
                select_feats = []
                select_atts = []
                select_labels = []
                ...
                for p in neta.parameters():
                        p.requires_grad_(False)
                for p in netae.parameters():
                        p.requires_grad_(True)
                netae.zero_grad()
                i_feat, i_h, i_hs, i_hn = netae(feat_vec.cuda())
                j_feat, j_h, j_hs, j_hn = netae(select_feat_vec.cuda())
                v_hs = lam*i_hs+(1-lam)*j_hs
                v_att = neta(v_hs.cuda())
                # v_feat, v_h, v_hs, v_hn = netae(x_mean)
                loss_visual = mse1(i_feat.cuda(), feat_vec.cuda()) + mse1(j_feat.cuda(), select_feat_vec.cuda()) + mse1(
                        v_att.cuda(), v_att_vec.cuda())
                ...
                for p in netae.parameters():
                        p.requires_grad_(False)
                neta.zero_grad()
                i_att = neta(i_hs.cuda())
                j_att = neta(j_hs.cuda())
                loss_att = mse1(i_att.cuda(), att_vec.cuda()) + mse1(j_att.cuda(), select_att_vec.cuda())
