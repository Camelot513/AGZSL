...
matcontent = io.loadmat('/data/xbjin_data/lw/ZSLearning/AGZSL-main/dataset/xlsa/AWA2/res101.mat')

feature = matcontent['features'].T
print(feature.shape)
label = matcontent['labels'].astype(int).squeeze() - 1
matcontent = io.loadmat('/data/xbjin_data/lw/ZSLearning/AGZSL-main/dataset/xlsa/AWA2/att_splits.mat')
...
train_seen_label = label[test_seen_loc]
seenclasses = np.unique(train_seen_label)
test_seen_att = np.array(attribute[seenclasses])
print('Seen_att: ', test_seen_att.shape)
train_unseen_label = label[test_unseen_loc]
unseenclasses = np.unique(train_unseen_label)
test_unseen_att = np.array(attribute[unseenclasses])
...
# 对特征矩阵进行PCA降维
train_test_pca = PCA(n_components=2).fit_transform(train_test_data.data)
real_unseen_pca = train_test_pca[:test_unseen_num, :]
real_seen_pca = train_test_pca[test_unseen_num:test_unseen_num+test_seen_num, :]
sampled_pca = train_test_pca[test_unseen_num+test_seen_num:, :]
...
plt.scatter(sampled_pca[:, 0],sampled_pca[:, 1], color='#619505', s=10, linewidths=0, alpha=0.6, label='sampled_tsne') # tran sampled data, AWA2: s=3, linewidths=1, aloha-0.6"
plt.scatter(real_unseen_pca[:, 0], real_unseen_pca[:, 1], color='#f96441', s=10, linewidths=0, alpha=0.6, label='real_unseen_tsne') # test data,AWA2: s=4, linewidths=0, alpha=0.6
plt.scatter(real_seen_pca[:, 0], real_seen_pca[:, 1], color='#BE5A83', s=10, linewidths=0, alpha=0.6, label='real_seen_tsne') # test data,AWA2: s=4, linewidths=0, alpha=0.6
#plt.savefig('../CUB/unseen_feature_description.png', bbox_inches='tight')
# plt.legend(markerscale=2)
# plt.title('APY Semantic')
# plt.show()
# 将特征矩阵按类别划分
