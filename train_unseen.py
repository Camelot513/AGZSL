...
def compute_accuracy_all(test_att, att_all, test_visual_unseen, test_id_unseen, test_label_unseen,
        ...
        with torch.no_grad():
            _,_,test_visual_unseen_hs,_ = netAE(test_visual_unseen)
            _, _, test_visual_seen_hs, _ = netAE(test_visual_seen)

        acc_zsl,_,unseenacc_cls,_,unseenpred_cls = calc_accuracy(test_visual_unseen_hs, test_label_unseen, test_att, test_id_unseen,test_id_unseen)
        acc_seenAcc,_,seenacc_cls,_,_ = calc_accuracy(test_visual_seen_hs, test_label_seen, att_all, train_id, train_id)
        ...
        acc_gzsl_unseen, Ru, unseengeneralAcc_cls, unseenCos_cls, _ = calc_accuracy(test_visual_unseen_hs,
                                                                                    test_label_unseen, att_all_cls,
                                                                                    test_id_all, test_id_unseen,
                                                                                    cossim=True)
        ...
        acc_gzsl_seen, Rs, seengeneralAcc_cls, seenCos_cls, _ = calc_accuracy(test_visual_seen_hs, test_label_seen,
                                                                              att_all_cls, test_id_all, train_id,
                                                                              cossim=True)
...
for epoch in range(args.num_epochs):
        ...
        for i in range(1000):
                batch_visual, batch_att, batch_label = dataset.__our_getitem__(i, netAE) # __our_getitem__(i,ournet) use our net to process att and visual, and new att and visual
