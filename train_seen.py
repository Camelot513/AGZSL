...
class train(object):
    ...
    def loadDataFromXlsa(self,dataType):
        """
            Load Data from Xlsa:
            ------

        """
        print('load from xlsa')
        attSplit = loadmat('./dataset/xlsa/' + self.cfg['dataset']['name'] + '/att_splits.mat')
        res101 = loadmat('./dataset/xlsa/' + self.cfg['dataset']['name'] + '/res101.mat')
        labels = res101['labels'].astype(int).squeeze() - 1
        train_seen_idx = attSplit['trainval_loc'].squeeze() - 1
        test_seen_dataLoc = attSplit['test_seen_loc'].squeeze() - 1
        test_unseen_dataLoc = attSplit['test_unseen_loc'].squeeze() - 1
        all_features = res101['features'].T
        if dataType=='train_seen':
            imgfs = np.array(all_features[train_seen_idx])
            labels = np.array(labels[train_seen_idx])
        elif dataType=='test_seen':
            imgfs = np.array(all_features[test_seen_dataLoc])
            labels = np.array(labels[test_seen_dataLoc])
        elif dataType=='test_unseen':
            imgfs = np.array(all_features[test_unseen_dataLoc])
            labels = np.array(labels[test_unseen_dataLoc])
    ...
    def trainModel(self,chkFile,recordFile,resume=False):
        ...
        trainData = self.loadDataFromXlsa(dataType='train_seen')
        testSeenData = self.loadDataFromXlsa(dataType='test_seen')
        testUnseenData = self.loadDataFromXlsa(dataType='test_unseen')
        bestHs = 0
...
if __name__ =="__main__":
    ...
    parser.add_argument('--modelChk',type=str,default='./chk/chk',help='model chkpoint file')
    parser.add_argument('--recordFile',default='./chk/record.txt',type=str,help='record loss and acc file')
    parser.add_argument('--summaryFolder',default='../summary/',type=str,help='tensorboardX file folder')
    ...
    parser.add_argument('--train',default=True,action='store_true',help='train or just eval')
